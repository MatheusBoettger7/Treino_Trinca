(function(){
  ...
  function editorExerciseImage(it){
    try{
      const local=window.appExerciseMedia?.[it.name]?.images||{};
      if(local.peak||local.start||local.main)return local.peak||local.start||local.main;
      if(it.repdbId&&typeof RepDB!=='undefined'&&RepDB.loaded){
        const ex=RepDB.get(it.repdbId);
        const src=ex?RepDB.image(ex,'peak')||RepDB.image(ex,'start'):'';
        if(src)return src;
      }
      if(typeof exerciseImage==='function'){
        const src=exerciseImage(it.name,0,'peak');
        if(src)return src;
      }
      if(it.repdbId)return `https://exercise-dataset.com/images/flat/${encodeURIComponent(it.repdbId)}-peak.webp`;
    }catch(e){}
    return '';
  }

  function rowHtml(it,i,total){const img=editorExerciseImage(it);return `<div class="editor-exercise"><div class="editor-exercise-head"><div><div class="editor-exercise-title"><strong>${i+1}. ${escLocal(it.name)}</strong>${it.repdbId?'<span class="editor-linked">🔗 RepDB</span>':''}</div><div class="muted small editor-image-label">${img?'Imagem do exercício':'Imagem indisponível'}</div></div>${img?`<img class="editor-exercise-visual" src="${escLocal(img)}" alt="${escLocal(it.name)}" loading="lazy" referrerpolicy="no-referrer">`:''}</div><div class="editor-grid"><label>Exercício<input id="ename_${i}" value="${escLocal(it.name)}" data-editor-field="name" data-index="${i}"></label><label>Séries<input type="number" min="1" max="20" id="esets_${i}" value="${Number(it.sets)||1}" data-editor-field="sets" data-index="${i}"></label><label>Repetições<input id="ereps_${i}" value="${escLocal(it.reps||'')}" data-editor-field="reps" data-index="${i}"></label></div><div class="editor-order"><button type="button" class="secondary" data-editor-action="pick" data-index="${i}">🔎 Escolher da biblioteca</button>${i?`<button type="button" class="secondary" data-editor-action="move" data-index="${i}" data-delta="-1">↑</button>`:''}${i<total-1?`<button type="button" class="secondary" data-editor-action="move" data-index="${i}" data-delta="1">↓</button>`:''}<button type="button" class="secondary" data-editor-action="remove" data-index="${i}">🗑️ Remover</button></div></div>`}
